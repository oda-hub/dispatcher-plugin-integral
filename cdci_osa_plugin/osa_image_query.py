...
class OsaImageProduct(ImageProduct):
    ...
    def __init__(self,
        ...
        if meta_data == {} or meta_data is None:
            self.meta_data = {'product': 'osa_mosaic',
                              'instrument': 'integral', 'src_name': ''}
        else:
    ...
    def build_from_ddosa_skyima(cls, file_name=None, skyima=None, ext=None, file_dir=None, prod_prefix=None, meta_data={}):
        try:
            data = NumpyDataProduct.from_fits_file(
                skyima, ext=None, meta_data=meta_data)
        except Exception as e:
            logger.error(f"issue while reading skyima {skyima}: {repr(e)}")
            raise DataAccessIssue(
                f"issue while reading skyima {skyima}: {repr(e)}")
...
class OsaMosaicQuery(ImageQuery, OsaQuery):
    ...
    def get_data_server_query(self, instrument, config=None):
        ...
        if (isinstance(self, JemxMosaicQuery)):
            ...
            target, modules, assume = self.set_instr_dictionaries(
                extramodules, scwlist_assumption, E1, E2, osa_version)

        q = OsaDispatcher(config=config, target=target, modules=modules,
                          assume=assume, inject=inject, instrument=instrument)
    ...
    def process_product_method(self, instrument, prod_list, api=False, **kw):
        ...
        query_image.add_url_to_fits_file(
        ...
        if api == False:
            ...
            query_out.prod_dictionary['numpy_data_product_list'] = [
                query_image.data]
            query_out.prod_dictionary['catalog'] = query_catalog.catalog.get_dictionary(
            )
...
class IsgriMosaicQuery(OsaMosaicQuery):
    ...
    def build_product_list(self, instrument, res, out_dir, prod_prefix=None, api=False):
        ...
        osa_catalog = CatalogProduct('mosaic_catalog',
                                     catalog=OsaIsgriCatalog.build_from_ddosa_srclres(
                                         res.srclres),
    ...
    def set_instr_dictionaries(self, extramodules, scwlist_assumption, E1, E2, osa_version="OSA10.2"):
        ...
        if osa_version_base == "OSA10.2":
            ...
            modules = ["git://ddosa/staging-1-3", "git://findic/staging-1-3-icversion",
                       "git://ddosa11/staging-1-3"] + extramodules + ['git://ddosa_delegate/staging-1-3']
        ...
        assume = ['ddosa.ImageGroups(input_scwlist=%s)' % scwlist_assumption[0],
                  ...
                  'ddosa.ImageBins(use_ebins=[(%(E1)s,%(E2)s)],use_version="onebin_%(E1)s_%(E2)s")' % dict(
                      E1=E1, E2=E2),
        ...
        if osa_subversion != "default-isdc":
            assume.append(
                f'ddosa.ICRoot(use_ic_root_version="{osa_subversion}")')
...
class JemxMosaicQuery(OsaMosaicQuery):
    ...
    def get_dummy_products(self, instrument, config, out_dir='./', api=False):
        ...
        catalog = CatalogProduct(name='mosaic_catalog',
                                 catalog=BasicCatalog.from_fits_file(
                                     '%s/jemx_query_catalog.fits' % dummy_cache),
                                 file_name='query_catalog',
                                 file_dir=out_dir)
    ...
    def build_product_list(self, instrument, res, out_dir, prod_prefix=None, api=False):
        ...
        if hasattr(res, 'skyima'):
            ...
            osa_catalog = CatalogProduct('mosaic_catalog',
                                         catalog=OsaJemxCatalog.build_from_ddosa_srclres(
                                             res.srclres),
                                         file_name='query_catalog',
                                         name_prefix=prod_prefix,
                                         file_dir=out_dir)
